export default function WhyArvestaDev() {
  const features = [
    {
      ...
      icon: (
        <svg className="w-6 h-6 text-white group-hover:text-[var(--color-primary)] transition-colors duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  <div 
    key={index} 
    style={{ backgroundImage: "url('/feature-bg.webp')" }}
    className="group relative min-h-[520px] bg-cover bg-center flex flex-col items-center justify-center text-center rounded-2xl border border-[var(--color-border)] transition-all duration-300 cursor-pointer hover:shadow-lg hover:-translate-y-1  overflow-hidden"
  >
    {/* Dark Tint Overlay */}
    <div className="absolute inset-0 bg-black/30 transition-opacity duration-300 group-hover:bg-black/60"></div>

    {/* Content Container (z-10 keeps it above the tint) */}
    <div className="relative z-10 flex flex-col items-center p-6 w-full">
    
      <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-[var(--color-primary)] shadow-md group-hover:bg-white transition-colors duration-300 text-white">
        {feature.icon}
      </div>
      ...
      <p className="text-sm leading-relaxed text-gray-200 bg-transparent backdrop-blur-[3px] py-2 px-3">
        {feature.description}
      </p>
    </div>
  </div>
